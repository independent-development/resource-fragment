...
async function encode() {
  ...
  /** 计算切片大小 **/
  const unit_size = 2 * parseInt(resource_buffer.length / 10);
  record.unit_size = unit_size;
  /** 计算头部的二进制填充 **/
  const header_buffer_pedding = new Uint8Array(Array(unit_size).fill(1));
  /** 文件尾需要填充的字符串 **/
  const footer_buffer_pedding = new Uint8Array(Array(unit_size - resource_buffer.length % unit_size).fill(1));
  /** 获取到最终资源的内容 **/
